...
# ===== ADICIONAR ESTA FUNÇÃO NO bot.py =====
# Colocar APÓS a função 'comandos' e ANTES da função 'pagar'

async def test_utm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Comando para testar o rastreamento UTM"""
    user_id = str(update.message.from_user.id)
    bot_id = context.bot_data['id']
    
    # Busca dados de tracking salvos
    tracking_data = manager.get_user_tracking(user_id, bot_id)
    
    message = "🧪 <b>TESTE DE RASTREAMENTO UTM</b>\n\n"
    
    if tracking_data:
        message += "✅ <b>Dados UTM encontrados:</b>\n"
        message += f"• User ID: {tracking_data.get('user_id', 'N/A')}\n"
        message += f"• Bot ID: {tracking_data.get('bot_id', 'N/A')}\n"
        message += f"• FBCLID: {tracking_data.get('fbclid', 'N/A')}\n"
        message += f"• UTM Source: {tracking_data.get('utm_source', 'N/A')}\n"
        message += f"• UTM Campaign: {tracking_data.get('utm_campaign', 'N/A')}\n"
        message += f"• UTM Medium: {tracking_data.get('utm_medium', 'N/A')}\n"
        message += f"• UTM Content: {tracking_data.get('utm_content', 'N/A')}\n"
        message += f"• UTM Term: {tracking_data.get('utm_term', 'N/A')}\n"
        message += f"• SCK: {tracking_data.get('sck', 'N/A')}\n"
        message += f"• SRC: {tracking_data.get('src', 'N/A')}\n"
        message += f"• Criado em: {tracking_data.get('created_at', 'N/A')}\n"
    else:
        message += "❌ <b>Nenhum dado UTM encontrado</b>\n"
        message += "Use um link com parâmetros UTM para testar.\n\n"
        message += "Exemplo:\n"
        message += f"<code>t.me/{context.bot.username}?start=fbclid-ABC123_utm-source-facebook_utm-campaign-teste</code>\n"
    
    # Testa conexão com UTMIFY
    message += "\n📡 <b>Teste de conexão UTMIFY:</b>\n"
    ...
    try:
        ...
        if config_data.get('utmify_token'):
            ...
            # Testa enviando um pedido de teste
            test_order = {
                "orderId": f"TEST-{user_id}-{int(time.time())}",
                "platform": config_data.get('platform_name', 'NGKPay'),
                "paymentMethod": "pix",
                "status": "waiting_payment",
                "createdAt": tracker._get_utc_time(),
                "approvedDate": None,
                "refundedAt": None,
                "customer": {
                    "name": f"TESTE {user_id}",
                    "email": f"teste-{user_id}@teste.com",
                    "phone": None,
                    "document": None,
                    "country": "BR",
                    "ip": "127.0.0.1"  # IP fixo
                },
                "products": [{
                    "id": "TEST-001",
                ...
                "isTest": True  # IMPORTANTE: Marca como teste
            }
            
            result = tracker.send_order(test_order)
            
            if result.get('success'):
                message += "✅ Conexão OK! Pedido de teste enviado.\n"
                message += f"Order ID: TEST-{user_id}-{int(time.time())}\n"
                message += "\n⚠️ <b>Nota:</b> Este é um pedido de TESTE (R$ 1,00)"
            else:
                message += f"❌ Erro: {result.get('error', 'Desconhecido')}\n"
                if result.get('status'):
                    message += f"Status HTTP: {result.get('status')}\n"
        else:
            message += "❌ Token UTMIFY não configurado no config.json\n"
    ...
    # Instruções finais
    message += "\n📝 <b>Como usar:</b>\n"
    message += "1. Clique em um link com UTMs\n"
    message += "2. Use /test_utm novamente\n"
    message += "3. Verifique se os dados aparecem\n"
    
    await update.message.reply_text(message, parse_mode='HTML')


# ===== AGORA ADICIONE O HANDLER =====
# No função run_bot, adicione esta linha APÓS os outros CommandHandlers:
# Por exemplo, após esta linha:
# bot_application.add_handler(CommandHandler('comandos', comandos))
# ADICIONE:
# bot_application.add_handler(CommandHandler('test_utm', test_utm))
