...
class UTMifyTracker:
    def __init__(self, api_token, platform_name="NGKPay"):
        ...
        # NOVO: IP padrão para quando não temos o IP real
    ...
    def send_order(self, order_data):
        ...
        try:
            # NOVO: Debug do que está sendo enviado
            print(f"[UTMIFY DEBUG] Enviando order: {json.dumps(order_data, indent=2)}")
            ...
            # NOVO: Debug da resposta
            print(f"[UTMIFY DEBUG] Status: {response.status_code}")
            print(f"[UTMIFY DEBUG] Resposta: {response.text}")
    ...
    def create_waiting_payment(self, user_id, bot_id, plan, order_id):
        ...
        # NOVO: Debug dos dados de tracking
        print(f"[UTMIFY DEBUG] Tracking data para user {user_id}: {tracking}")
        
        # CORREÇÃO PRINCIPAL: Usa IP padrão se não tiver IP salvo
        customer_ip = tracking.get('ip_address') or self.default_ip
        ...
        order_data = {
            ...
            "customer": {
                ...
                "document": None,
                "country": "BR",
                "ip": customer_ip  # CORREÇÃO: Sempre terá um valor válido
            },
            "products": [{
                "id": f"plan_{bot_id}",
